...
export default function AircraftTypeSelect({ value, onChange, companySlug, manufacturerId }: AircraftTypeSelectProps) {
  const {
    data: aircraftTypes,
    isLoading: isAircraftTypesLoading,
    isError: isAircraftTypesError,
  } = useGetAircraftTypes(companySlug, undefined, manufacturerId);
    ...
    <FormItem className="space-y-2">
      ...
      >
        <FormControl>
          <SelectTrigger>
            <SelectValue
              placeholder={manufacturerId ? 'Seleccione el tipo de aeronave...' : 'Seleccione un fabricante primero...'}
            />
          </SelectTrigger>
        </FormControl>
        <SelectContent>
