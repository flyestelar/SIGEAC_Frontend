...
const fetchDocument = async ({ company, fileName, origin = 'sms' }: UseGetDocumentProps): Promise<string> => {
  const encodedDocumentPath = btoa(fileName);
  const response = await axiosInstance.get(`${company}/${origin}/document/${encodedDocumentPath}`, {
    responseType: 'blob',
  });
