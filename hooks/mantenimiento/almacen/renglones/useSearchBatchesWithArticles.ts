...
const searchBatchesWithArticles = async ({
  ...
  const { data } = await axios.get(`/${company}/${location_id}/search-articles-with-batch`, {
    params: { part_number },
  });
  return data;
};

export const useSearchBatchesWithArticles = (part_number?: string) => {
  const { selectedCompany, selectedStation } = useCompanyStore();
  return useQuery<BatchWithArticles[], Error>({
    queryKey: ['search-batches-with-articles', selectedCompany?.slug, selectedStation, part_number],
    queryFn: () =>
      searchBatchesWithArticles({
