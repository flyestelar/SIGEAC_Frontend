...
export const useCashMovementForAircraft = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await axiosInstance.post(`/${company}/cash-movement-aircraft/${acronym}/expenses`, data);
      return response.data as CashMovement;
    },
    onSuccess: (newMovement) => {
      queryAircraft.setQueryData(['aircrafts', 'movements'], (old: CashMovement[] | undefined) =>
