...
export const columns: ColumnDef<User>[] = [
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableColumnHeader filter column={column} title="Usuario" />,
    cell: ({ row }) => {
  ...
  {
    accessorKey: 'email',
    header: ({ column }) => <DataTableColumnHeader filter column={column} title="Email" />,
    cell: ({ row }) => {
      const item = row.original;
      return (
        <div className="flex items-center justify-center">
      ...
      return (
        <div className="flex items-center justify-center">
          {item.isActive ? (
            ...
            <Badge className="bg-rose-500">INACTIVO</Badge>
          )}
        </div>
      );
    },
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col gap-2 justify-center">
          {item && item.roles && item?.roles?.length < 3
            ? item.roles.map((rol) => (
                <div
                  onClick={() => redirect('/administracion/usuarios_permisos/roles')}
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <UserDropdownActions user={user} companies={companies} />;
    },
  },
];
