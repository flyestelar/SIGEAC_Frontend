...
export function ControlCard({ computed, href, averages }: ControlCardProps) {
  ...
  return (
    <Link href={href} className="block">
      <Card className="rounded-xl shadow-md h-full flex flex-col group cursor-pointer overflow-hidden transition-all hover:shadow-lg">
        ...
        <CardFooter className="flex flex-row items-center justify-between px-5 py-4">
          ...
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.preventDefault(); e.stopPropagation(); router.push(`/${selectedCompany?.slug}/planificacion/control_mantenimiento/${control.id}/editar`); }}>
