...
import { AlertCircle, Calendar, FileText, RefreshCw, Search, ShieldCheck } from 'lucide-react';
...
export function DirectivesTabContent({ items }: DirectivesTabContentProps) {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      if (directive?.ad_number?.toLowerCase().includes(q)) return true;
      if (directive?.subject_description?.toLowerCase().includes(q)) return true;
      if (directive?.authority?.toLowerCase().includes(q)) return true;
      return false;
    });
  }, [items, search]);
    ...
    <div className="space-y-3">
      ...
      {filteredItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-1.5 py-12 text-muted-foreground">
          ...
          <p className="text-sm">
            {search
              ? 'No se encontraron directivas con ese término de búsqueda.'
              : 'No hay directivas de aeronavegabilidad asociadas a esta orden.'}
          </p>
        </div>
      ) : (
        <div className="divide-y">
          {filteredItems.map((directiveItem) => {
