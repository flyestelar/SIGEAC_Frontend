  ...
  return (
    <div className="flex items-center gap-1 px-3 py-1">
      ...
      <div className="flex items-center gap-1">
        {[0, 1, 2].map((i) => (
          <div key={i} className={cn('h-1.5 w-1.5 rounded-full', dotColor)} />
        ))}
      </div>
      <div className="h-px flex-1 bg-border/60" />
    </div>
...
function SlotIdentity({
  ...
  return (
    <div className="flex items-center gap-2 px-4 pt-3">
      ...
      >
        <MapPinned className={cn('h-3.5 w-3.5', isEmpty ? 'text-sky-600' : 'text-muted-foreground')} />
      </div>
      <div className="flex min-w-0 items-baseline gap-1.5">
        <p className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">Slot</p>
        <span className="font-mono text-sm font-semibold text-foreground">{position}</span>
        {ataChapter && (
          <>
            <span className="text-[11px] text-border">|</span>
            <span className="font-mono text-[11px] text-muted-foreground">ATA {ataChapter}</span>
          </>
        )}
      </div>
    </div>
...
export function HardTimeCard({
      ...
      >
        ...
        <div className="space-y-3 px-4 pb-3">
          {pendingRequest ? (
            ...
            <div className="flex flex-col items-center gap-2 rounded-md border border-dashed border-sky-400/30 bg-background/50 py-4">
              <div className="flex h-8 w-8 items-center justify-center rounded-full border border-sky-400/30 bg-sky-500/10">
              ...
              {!pendingRequest && (
                <Button
                  size="sm"
                  variant="outline"
                  className="h-7 gap-1.5 border-sky-500/30 px-3 text-[11px] text-sky-700 hover:bg-sky-500/10 dark:text-sky-300"
                  onClick={(e) => {
                    e.stopPropagation();
                    onInstall?.();
                  }}
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <Badge
              variant="outline"
              className="h-5 gap-1 border-sky-500/20 bg-sky-500/10 px-1.5 text-[10px] text-sky-700 dark:text-sky-300"
            >
  ...
  return (
    ...
    >
      ...
      <div className="mx-3 mb-3 overflow-hidden rounded-md border border-border/60 bg-background/70">
        {/* Part header */}
        <div className="flex items-start justify-between gap-2 border-b border-border/40 px-3 py-2.5">
          <div className="flex min-w-0 items-start gap-2.5">
            ...
            >
              <LevelIcon className={cn('h-3.5 w-3.5', cfg.iconText)} />
            </div>
            <div className="min-w-0 space-y-0.5">
              ...
              {component.batch?.name &&
                ...
                component.batch.name !== component.description && (
                  <p className="truncate text-[10px] text-muted-foreground/70">{component.description}</p>
                )}
            </div>
          </div>
              ...
              <div className="space-y-3">
                {intervals.map((interval, intervalIdx) => (
                  <div
                    ...
                    className="space-y-2 border-b border-border/30 pb-2 last:border-b-0 last:pb-0"
                  >
                    <div className="flex items-center gap-1.5">
                      <GripHorizontal className="h-3 w-3 shrink-0 text-muted-foreground/40" />
        ...
        <div className="flex items-center justify-between gap-2 border-t border-border/40 px-3 py-2">
          ...
          <div className="flex shrink-0 items-center gap-1" onClick={(e) => e.stopPropagation()}>
            ...
            <Button
              size="sm"
              variant="outline"
              className="h-7 gap-1 border-amber-500/30 px-2.5 text-[11px] text-amber-600 hover:bg-amber-500/10 dark:text-amber-400"
              onClick={(e) => {
                e.stopPropagation();
                onUninstall?.();
              }}
