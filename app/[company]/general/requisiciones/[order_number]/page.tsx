...
import { FileText, Loader2, Trash2, User, Image as ImageIcon, EyeIcon } from 'lucide-react';
      ...
      <Card className="max-w-7xl mx-auto">
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          {data?.image && (
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-sm text-muted-foreground">Imagen adjunta a la requisición</p>
            </div>
          )}
              ...
              data.batch.map((batch) => (
                <Card key={batch.id} className="w-full max-w-xl">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-2xl text-center">{batch.name}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {batch.batch_articles.map((article, index) => (
                      <div key={`${article.article_part_number}-${index}`} className="space-y-4">
                            ...
                            {article.image ? (
                              <>
                                <div className="max-w-xs">
                                  <Image
                                    src={`${process.env.NEXT_PUBLIC_STORAGE_BASE_URL}${article.image}`}
                                ...
                                <p className="text-sm text-muted-foreground mt-2">Imagen del artículo</p>
                              </>
                            ) : (
                        ...
                        {index < batch.batch_articles.length - 1 && <Separator className="my-4" />}
                      </div>
                    ))}
                  </CardContent>
                </Card>
        ...
        <CardFooter className="flex gap-2 justify-end">
          {Array.isArray(data?.document) &&
            data.document.map((doc, idx) => (
              <DocumentDisplayDialog key={idx} fileName={doc} isPublic={true} title={doc.split('/').pop()} />
            ))}
          <Button
