...
export const FilterPanel = React.memo(({ batches, filterState, filterActions, stats }: FilterPanelProps) => {
    ...
    <Card className="mb-4 overflow-hidden">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          >
            <span className="transition-all duration-200">{filtersExpanded ? 'Contraer' : 'Expandir'}</span>
            <ChevronDown
        ...
        {!filtersExpanded && (
          <div className="text-sm text-muted-foreground mt-2 animate-in fade-in-50 slide-in-from-top-2 duration-300">
            Mostrando <span className="font-medium text-foreground">{articleCounts.filteredArticles}</span> de{' '}
            <span className="font-medium text-foreground">{articleCounts.totalArticles}</span> artículos
            {hasActiveFilters && <span className="ml-2 text-blue-600">(filtros aplicados)</span>}
          </div>
        )}
      </CardHeader>
      ...
      >
        <CardContent className="space-y-3 pt-1">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            >
              <label className="text-sm font-medium text-muted-foreground">Zona de Almacén</label>
            ...
            >
              <label className="text-sm font-medium text-muted-foreground">Número de Parte</label>
          ...
          >
            <div className="text-sm text-muted-foreground">
              ...
              de <span className="font-medium text-foreground">{articleCounts.totalArticles}</span> artículos
              {hasActiveFilters && (
                <span className="ml-2 text-blue-600 animate-in fade-in-50 duration-300">(filtros aplicados)</span>
              )}
            </div>
            {hasActiveFilters && (
